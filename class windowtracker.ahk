class WindowTracker{
	...
	__New(win,nodisable:=0){
		hwnd:=Setup(win,nodisable),this.win:=win,this.hwnd:=hwnd,this.ahkid:="ahk_id" hwnd,this.type:=[],this.tracker:=[],this.resize:=[],WindowTracker.winlist[win]:=this,this.varlist:=[],WindowTracker.set:=new xml("window","lib\Window.xml")
	...
	AddHwnd(hwnd,resize){
		this.resize.Insert({control:hwnd,pos:Resize})
	}
	root(){
	...
	Show(title:="",position="",NoActivate:=""){
		...
		Gui,% this.win ":Show",%pos% %NoActivate% %AutoSize%,%title%
