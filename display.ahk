display(store){
	...
	if info:=recieve.ssn("//stream[@type='stderr']"){
		...
		sc:=csc()
		sc.2160(v.debugse.start,v.debugse.end)
		debug.disconnect()
		return
	}
	...
	if recieve.ssn("//property"){
		if property:=recieve.sn("//property"){
			...
			while,oo:=object.item[A_Index-1]{
				ea:=xml.ea(oo),value:=debug.decode(oo.text) ;("decode",oo.text)
				...
				while,des:=descendant.item[A_Index-1]{
					ea:=xml.ea(des),value:=debug.decode(des.text) ;("decode",des.text)
