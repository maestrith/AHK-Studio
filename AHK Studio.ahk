...
global v:=[],settings,files,menus,commands,positions,vversion,access_token,vault,preset,scintilla,bookmarks,cexml,notesxml
notesxml:=new xml("notes","lib\Notes.xml"),v.match:={"{":"}","[":"]","<":">","(":")",Chr(34):Chr(34),"'":"'","%":"%"},settings:=new xml("settings","lib\settings.xml"),commands:=new xml("commands","lib\commands.xml"),files:=new xml("files"),menus:=new xml("menus","lib\menus.xml"),bookmarks:=new xml("bookmarks","lib\bookmarks.xml"),positions:=new xml("positions","lib\positions.xml"),vversion:=new xml("version","lib\version.xml"),access_token:=settings.ssn("//github/@token").text,cexml:=new xml("code_explorer"),v.filescan:=[]
...
keywords(),plugins(),var(),gui(),ObjRegisterActive(pluginclass)
if(openfile){
	v.openfile:=openfile
...
#Include Close All.ahk
#Include Margin Left.ahk
#Include ProcessText.ahk
#Include QFS.ahk
#Include Var.ahk
#Include GUIKeep.ahk
#Include WinPos.ahk
