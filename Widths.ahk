Widths(){
	...
	attribute:=settings.Add({path:"gui"}),attribute.SetAttribute(A_ThisLabel,value),Resize("rebar")
	return
	24GuiClose:
	24GuiEscape:
	hwnd({rem:24})
	return
	hce:
	return Label("Hide_Code_Explorer"),Resize("rebar")
	hpe:
	return Label("Hide_Project_Explorer"),Resize("rebar")
}
Label(label){
	SetTimer,%Label%,10
	sleep,11
