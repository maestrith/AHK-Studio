Remove_Segment(){
	...
	v.filelist.remove(ssn(current,"@file").text),mainfile:=update({get:filename})
	...
	TV_Delete(ssn(current,"@tv").text),tv(ssn(current.previousSibling,"@tv").text),current.parentnode.removechild(current)
