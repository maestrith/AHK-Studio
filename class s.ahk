class s{
	...
	__New(window,info){
		...
		this.2036(width:=settings.ssn("//tab").text?settings.ssn("//tab").text:5),this.2124(1),this.2260(1),this.2122(5),color(this)
		this.2056(38,"Consolas"),this.2516(1)
		return this
	}
	clear(){
		for a,b in s.temp
	...
	delete(x*){
		this:=x.1
		if(!(this.sc=v.debug.sc&&v.debug.sc)){
			...
			v.debug:=[],resize("rebar")
		}
		DllCall("DestroyWindow",uptr,this.sc),csc("Scintilla1").2400,Resize("rebar")
	}
	__Get(x*){
