Class Code_Explorer{
	static explore:=[],TreeView:=[],sort:=[],function:="OUm`n)^\s*((\w|[^\x00-\x7F])+)\((.*)\)(\s+;.*)?\n?[\s*]?\{",label:="Om`n)^\s*((\w|[^\x00-\x7F])+):[\s+;]",class:="Om`ni)^[\s*]?(class[\s*](\w|[^\x00-\x7F])+)",functions:=[],variables:=[],varlist:=[]
	scan(node){
		...
		if(!cce:=ssn(main,"descendant::file[@file='" filename "']"))
	...
	Refresh_Code_Explorer(){
		...
		if(Focus="SysTreeView321"){
			...
			if(type.nodename="folder"){
				...
				for a,b in StrSplit("Close Project,Open,Rename,Remove Segment,,Copy File Path,Copy Folder Path,Width,Hide/Show Icons,File Icon",",")
					Menu,rcm,Add,%b%,rcm
			Menu,rcm,show
			Menu,rcm,DeleteAll
			Menu,rcm,Delete
			...
			if(A_ThisMenuItem="Open")
				...
				top:=settings.add2("icons/pe"),top.SetAttribute("show",_:=xml.ea(top).show?0:1),TVIcons()
