class github{
	...
	getref(repo){
		url:=this.url "/repos/" this.owner "/" repo "/git/refs" this.token ;GET /repos/:owner/:repo/git/refs
	...
	blob(repo,text){
		url:=this.url "/repos/" this.owner "/" repo "/git/blobs" this.token ;POST /repos/:owner/:repo/git/blobs
		text:=this.utf8(text)
