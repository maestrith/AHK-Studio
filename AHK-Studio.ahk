...
OR PERFORMANCE OF THIS SOFTWARE. 
	...
	Setup(11),Hotkeys(11,{"Esc":"11Close"}), Version:="1.003.13"
	Gui,Margin,0,0
...
Backspace(sub:=1){
	...
	if(!InStr(focus,"Scintilla")){
		Send,{%A_ThisHotkey%}
		return
	}if(sc.2570=1&&!v.Options.Smart_Delete){
		Send,{%send%}
		return
	}
...
Command_Help(){
	...
	if(RegExMatch(line,"O)^\s*#(\w*)",found))
	...
	else if(RegExMatch(commands.SSN("//Commands/Commands").text,"\b(" found1 "|" sc.GetWord() ")\b",ff)){
		found1:=ff1
		...
		OpenHelpFile(url)
	}else if(RegExMatch(sc.GetWord(),"Oi)A_(\w*)",found)){
		OpenHelpFile("mk:@MSITStore:C:\Program%20Files\AutoHotkey\AutoHotkey.chm::/docs/Variables.htm#" found.1)
	}else
		m("The word: " Chr(34) found1 Chr(34) " was found and was not handled by AHK Studio.","If this is a command please let maestrith know.","Opening the help file"),OpenHelpFile("mk:@MSITStore:C:\Program%20Files%20(x86)\AutoHotkey\AutoHotkey.chm::/docs/AutoHotkey.htm")
	return
}
Compile_AHK_Studio(){
...
ContextMenu(){
	...
	while(aa:=all.item[A_Index-1]),ea:=XML.EA(aa),pea:=XML.EA(parent:=aa.ParentNode){
		...
		if(aa.NodeName="Separator"){
			...
			if(((code:=UnRedo[ea.name])&&!sc[code])||(sc.2008=sc.2009&&ea.name~="Copy|Cut|Delete")||Clipboard=""&&ea.name="Paste"||sc.2143=0&&sc.2145=sc.2006&&ea.name="Select All"||ea.name="Open Folder"&&Current(2).untitled)
	...
	*/
	...
	__New(option:=0){
		...
		*/
		...
		;/setup glabels
		for a,b in ["Options","Menus","Auto Insert","Edit Replacements","Manage File Types","Theme"] ;<----Add tab stuffs here make sure to move the icon list to the end
...
Toggle_Multiple_Line_Comment(){
	sc:=csc(),top:=sc.2010(sc.2143),bottom:=sc.2010(sc.2145)
	if(top=11&&bottom!=11||bottom=11&&top!=11)
	...
	Loop,% bottomline-topline{
		if(sc.2010(sc.2128((A_Index-1)+topline))=11)
			return m("With your current selection I can not create a Multiple Line Comment")
		;else
			;m(sc.2010(sc.2128((A_Index-1)+topline)),(A_Index-1)+topline)
	}
	sc.2078(),sc.Enable(),indent:=Settings.Get("//tab",5)
	;here!!!!
	return m(top,bottom,topline,bottomline),sc.Enable(1),sc.2079
	/*
