...
OR PERFORMANCE OF THIS SOFTWARE. 
	...
	setup(11),Hotkeys(11,{"Esc":"11GuiClose"}), Version:="1.002.29"
	Gui,Margin,0,0
...
Encode(tt,encoding:="UTF-8"){
	length:=VarSetCapacity(text,strput(tt,encoding)),StrPut(tt,&text,length,encoding)
	return text
}
Escape(){
	sc:=csc()
	...
	if(sel){
		...
		if(ea.sc)
			...
			if(!ea.sc){
				sc.2358(0,0)
				Sleep,20
				doc:=sc.2357,sc.2376(0,doc),node.SetAttribute("sc",doc),tt:=update({get:ea.file}),sc.2037(65001),txt:=Encode(tt,enc:=ssn(node,"@encoding").text),sc.2181(0,&txt),Update({encoding:enc,file:ea.file}),sc.2175(),Set(sc)
			}else
