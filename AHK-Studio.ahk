...
Duplicates(){
	static LastSearch
	sc:=csc(),sc.2500(3),sc.2505(0,sc.2006),dup:=[],search:=Trim(sc.TextRange((start:=sc.2143),(end:=sc.2145)),"`n`t "),v.lastsearch:=search,v.selectedduplicates:=""
	if(end-start<2)
		return
	sc.2686(0,sc.2006),sc.2500(3),sc.2198(v.Options.Match_Any_Word?0:0x2)
	len:=StrPut(search,"UTF-8")-1,obj:=v.duplicateselect[sc.2357]:=[] ;clean this up
	count:=0
	while(found:=sc.2197(len,[search]))>=0{
		sc.2686(found+1,sc.2006),obj[found]:=len,count++
		/*
			new:=main.Under(top,"select",{start:found,end:found+len})
			if(found=start)
				new.SetAttribute("sel",1)
		*/
	}
	if(count>1){
		for a,b in obj
...
class GUIKeep{
	...
	Show(name,position:="",NA:=0,Select:=0){
		...
		while,this:=GUIKeep.Showlist.pop(){
			position:=settings.SSN("//gui/position[@window='" this.win "']").text,position:=position?position:defpos,win:=[]
			for a,b in ["x","y","w","h"]
				RegExMatch(position,"Oi)" b "(-?\d*)\b",found),win[b]:=found.1
			if(!Displays.SSN("//*[@l<" win.x " and @r>" win.x " and @t<" win.y " and @b>" win.y "]")){
		...
		if(obj:=list[x])
			for c,d in s.ctrl{
				/*
					if(IsObject(obj))
						d[obj.1](obj.2,obj.3)
					;m(list[x].1,list[x].2,"","",v.Options[x]?(list[x].2?list[x].2:1):0)
					else
				*/
				d[obj](v.Options[x])
			}
...
ToggleDuplicate(){
	...
	if(!select.end)
		return
	/*
		sometimes when changing or adding duplicates it will remove underlines...not sure why.
		probably uppos stuff.
	*/
	Loop,% sc.2570{
	...
	;t(pos,A_TickCount,"time:1")
	return
	/*
		if(node:=main.SSN("//*[@start<='" pos "' and @end>='" pos "']")){
			ea:=xml.EA(node)
			if(ea.sel){
				if(main.SN("//*[@sel]").length>1){
					Loop,% sc.2570{
						start:=sc.2585(A_Index-1)
						if(start=ea.start){
							node.RemoveAttribute("sel"),sc.2671(A_Index-1)
							Break
						}
					}
				}
			}else if(!ea.sel){
				node.SetAttribute("sel",1),sc.2573(ea.start,ea.end)
				main.Transform()
				m("here?",main[],"",node.xml,"","",ea.sel)
			}
		}
	*/
	/*
	...
	*/
	/*
		if node has @sel
			check to see if there are more than 1 @sel and if there is de-select it
		if node not has @sel
			add @sel and select it
		t(main[],pos)
	*/
	if(sc.2507(3,pos)){
		start:=end:=pos
