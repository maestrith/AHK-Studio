...
OR PERFORMANCE OF THIS SOFTWARE. 
	...
	setup(11),Hotkeys(11,{"Esc":"11GuiClose"}), Version:="1.002.27"
	Gui,Margin,0,0
...
FEUpdate(file:="",Redraw:=0){
	...
	if((all:=files.find("//main/@file",file,"descendant::file[@tv]")).length)
		while(aa:=all.item[A_Index-1]),ea:=xml.ea(aa){
			if(ea.file~="i)flan|spoop")
				m("Should have something coming after this",ea.file,FileExist(ea.file),aa.xml)
			if(!FileExist(ea.file))
				Default("SysTreeView321"),TV_Delete(ea.tv),m(aa.xml),aa.ParentNode.RemoveChild(aa),m(aa.xml,"Should Be Gone")
		}
	if(Redraw)
		Default("SysTreeView321"),TV_Delete(),list:=files.sn("//main/file")
	else
		list:=files.find("//file/@file",file,"descendant-or-self::file")
	...
	all:=files.find("//main/@file",file,"descendant::*[not(@tv)]")
	while,aa:=all.item[A_Index-1],ea:=xml.ea(aa){
