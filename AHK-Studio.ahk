...
FileCheck(file:=""){
	...
	,scidate:=20180209111407,XMLFiles:={menus:[20180209133902,"lib/menus.xml","lib\Menus.xml"]}
...
class GUIKeep{
	...
	__Get(){
		...
		if(Settings.SSN("//options/@Add_Margins_To_Windows").text!=1)
			Gui,%win%:Margin,0,0
		Gui,%win%:Font,% "c" info.color " s10",Consolas ;% info.font
		...
		for a,b in info{
			...
			if(i.1="sc"){
				...
				if(node)
					new:=this.XML.Under(node,"control",{hwnd:hwnd,class:Class Count,type:RegExReplace(Class,"i)(Sys|\d)"),label:label1})
				else
					new:=this.XML.Add("control",{hwnd:hwnd,class:class count,label:label1,type:RegExReplace(Class,"i)(Sys|\d)")},,1)
		...
		Node:=this.XML.SSN("//*[@var='" VarName "']"),Type:=SSN(Node,"@type").text
		Gui,% this.Win ":Default"
		if(Type~="i)\b(TreeView|ListView)\b")
			Gui,% this.Win ":" Type,% SSN(Node,"@hwnd").text
		...
		defpos:=position,this.GetPos(),pos:=this.resize=1?"":"AutoSize",this.name:=name,Sel:=Select,this.NA:=NA,Displays:=this.GetDisplays()
		if(this.resize=1)
			Gui,% this.win ":+Resize"
		...
		while(this:=GUIKeep.ShowList.Pop()){
			...
			Gui,% this.win ":Show",% position " " pos " Hide " NA,% this.name
			if(this.resize!=1)
...
Refresh_Code_Explorer(){
	Before:=SSN(Current(1).NextSibling,"@file").text,CurrentFile:=Current(2).File
