...
Class SettingsWindow{
	...
	*/
	__New(option:=0){
		...
		;this should only be done if something in the menus actually changes
		MenuWipe(1)
		...
		SettingsWindow.menu:=[],SettingsWindow.IL:=IL_Create(20),IL_Add(SettingsWindow.IL,"Shell32.dll",50),SettingsWindow.Icons:=[]
		...
		obj.add("Text,xs ys,Info On What Icon You Are Setting,w"
			  ...
			  ,"Button,x+M gSettingsEditHotkey,Set Icon (Enter),y"
		...
		SettingsWindow.CommandList:=CommandList:={"Add A New Menu":["!A","ANM"],"Move Menu Item Up":["^Up","Up"],"Move Menu Item Down":["^Down","Down"],"Add Separator":["!S","AddSeparator"],"Edit Hotkey":["Enter","SettingsEditHotkey"],"Re-Load Defaults":["","RD"],"Sort Menus":["","SortMenus"],"Sort Selected Menu":["","SortSelectedMenu"],"Remove Icon":["!^x","RemoveIcon"],"Remove All Icons From Current Menu":["","RAICM"],"Move Selected":["^M","MS"],"Show Options Tab":["!O","ShowOptionsTab"],"Add New Sub-Menu":["!U","AddNewSubMenu"],"Remove All Icons":["","RAI"],"Delete/Hide Items":["Delete","SettingsDelete"],"Change Icon":["!I","SWCI"]}
	...
	SelectMenuNode(node,Redraw:=0){
		...
		/*
			if the menu exists
				select it and send it to PopulateMenuItems()
			have this send to PopulateMenu
			then once that is done populating have it send to PopulateMenuItems
		...
		*/
		...
		SettingsWindow.Enable()
		/*
			new plan, create a method for ONLY populating the SysListView322
			keep a list of the current items in there
			pass it a node and have it populate using the @select nodes to select and remove the @select at once.
		*/
		/*
			static ItemList,il:=[]
			SettingsWindow.PopulatingMenus:=1,SettingsWindow.Disable(),Default("SysTreeView322","Settings")
			if(SSN(node,"descendant::*")||SSN(node,"@personal")){
				...
				while(ll:=list.item[A_Index-1]),ea:=XML.EA(ll){
					if(!SN(ll,"descendant::*").length&&!ea.personal){
						...
						else if(selcount=1&&ea.select)
							select:="Select Vis Focus"
						else if(selcount>1&&index&&ea.select)
							select:="Select"
						else
							select:=""
				...
				if(tv!=TV_GetSelection()||Redraw){
					ItemList:=[],TV_Modify(tv,"Select Vis Focus"),list:=SN(node.ParentNode,"*"),Default("SysListView322","Settings"),LV_Delete(),selcount:=SN(node,"*/@select").length
					while(ll:=list.item[A_Index-1]),ea:=XML.EA(ll)
		...
		*/
		...
		return SettingsWindow.SelectMenuNode(new,1)
		/*
			Default("SysListView322","Settings"),LV_Delete(),Default("SysTreeView322","Settings"),TV_Delete(),all:=menus.SN("//main/descendant::menu")
			while(aa:=all.item[A_Index-1]),ea:=XML.EA(aa){
				if(SSN(aa,"descendant::*")||ea.personal)
					aa.SetAttribute("tv",TV_Add(RegExReplace(ea.clean,"_"," "),SSN(aa.ParentNode,"@tv").text,SettingsWindow.Icon(ea)))
			}if(tv:=menus.SN("//*[@select=1]/@tv").text)
				TV_Modify(tv,"Select Vis Focus")
			return ;
		*/
		SortMenus:
