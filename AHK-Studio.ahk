...
Update(info){
	...
	if(info.sc){
		...
		if(!item)
			return
		if(update[item]=Update({encoded:Current(3).file}))
			return
		if(updated[item]=""){
...
/*
	Update(info){
		...
		if(info.sc){
			...
			if(!item)
				return
			if(update[item]=sc.getplain(encoding[item]))
				return
			if(updated[item]=""){
				Default("SysTreeView321"),TV_Modify(ea.tv,"","*" (v.options.Hide_File_Extensions?ea.nne:ea.filename)),updated[item]:=1,WinSetTitle(1,ea)
			}if(v.options.disable_line_status!=1){
			...
			updated[item]:=1,update[item]:=sc.getplain(encoding[item])
			return
		}
		return
	}
