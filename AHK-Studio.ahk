...
ContextMenu(){
	...
	while(aa:=all.item[A_Index-1]),ea:=XML.EA(aa),pea:=XML.EA(parent:=aa.ParentNode){
		...
		if(aa.NodeName="Separator"){
			...
			if(((code:=UnRedo[ea.name])&&!sc[code])||(sc.2008=sc.2009&&ea.name~="Copy|Cut|Delete")||Clipboard=""&&ea.name="Paste"||sc.2143=0&&sc.2145=sc.2006&&ea.name="Select All")
...
Toggle_Multiple_Line_Comment(){
	...
	top:=sc.2010(sc.2143),bottom:=sc.2010(sc.2145)
	;here!!!!
	return m(top,bottom),sc.Enable(1),sc.2079
	/*
