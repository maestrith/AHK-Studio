		...
		while(aa:=all.item[A_Index-1]),ea:=xml.ea(aa){
			if(ea.move)
				...
				for a,b in {x:ea.x,y:ea.y,w:ea.w,h:ea.h}
					if(b!=""){
						GuiControl,%name%:MoveDraw,% ea.hwnd,% a (val:=(a~="y|h"?pos.h:pos.w)+b)
						if(ea.control~="Scintilla"&&a="w")
							s.ctrl[ea.hwnd].2274(val/2)
	}}}
	Show(name){
...
Color(con:=""){
	...
	for a,b in [[2040,25,13],[2244,3,0xFE000000],[2040,26,15],[2040,27,11],[2040,28,10],[2040,29,9],[2040,30,12],[2040,31,14],[2242,0,20],[2242,1,13],[2134,1],[2260,1],[2246,1,1],[2246,2,1],[2115,1],[2029,2],[2031,2],[2080,7,6],[2240,3,0],[2242,3,15],[2246,1,1],[2246,3,1],[2244,2,3],[2040,0,0],[2041,0,0],[2042,0,0xff],[2115,1],[2056,38,"Tahoma"],[2077,0,"abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ#_1234567890"],[2041,1,0],[4006,0,"ahk"],[2042,1,0xff0000],[2040,2,22],[2042,2,0x444444],[2040,3,22],[2042,3,0x666666],[2040,4,31],[2042,4,0xff0000],[2037,65001],[2132,v.options.Hide_Indentation_Guides=1?0:1],[2040,1,0],[2042,1,0x0000ff]]
		con[b.1](b.2,b.3)
...
EditHotkey(node,window){
	...
	nw:=new GUIKeep("Edit_Hotkey"),nw.add("Hotkey,w240 vhotkey gEditHotkey","Edit,w240 vedit gCustomHotkey","ListView,w240 h220,Duplicate Hotkey Definitions","Button,gEdit_HotkeyGuiEscape Default,Set Hotkey,y"),nw.show("Edit Hotkey")
	return
	EditHotkey:
...
Gui(){
	...
	}RefreshThemes(),debug.off(),TrayMenu(),hk(1),Hotkeys(1,Enter),(openfilelist)?Open(trim(openfilelist,"`n")):"",list:=settings.sn("//last/file")
	if(OpenAll.1){
		for a,b in OpenAll
...
Class GuiKeep{
	...
	__New(win,info*){
		...
		border:=0,this.hwnd:=hwnd(this.win) ;StrSplit(A_OSVersion,".").1=10?0:DllCall("GetSystemMetrics",int,32)
	...
	if(v.list.length>2){
		...
		while(GetKeyState("LButton")){
			...
			while,tt:=tops.item[A_Index-1],ea:=xml.ea(tt)
				tt.SetAttribute("tp",(mp.y+v.menu)/pos.h)
...
New_Segment(){
	...
	function:=Clean(nne),text:=(function~="i)^class_")?(m("Create Class called " (RegExReplace(SubStr(nne,InStr(nne," ")+1)," ","_")) "?","btn:ync")="Yes"?"Class " (RegExReplace(SubStr(nne,InStr(nne," ")+1)," ","_")) "{`n`t`n}":"",pos:=StrPut(nne "{`t","UTF-8")):(m("Create Function called " function "?","btn:ync")="Yes"?function "(){`n`t`n}":"",pos:=StrPut(function "(","UTF-8")-1),AddSegment(Filename,text,{start:pos,end:pos})
}
New(filename:="",text:=""){
