...
Close(x:=1,all:="",Redraw:=1){
	Save(),current:=IsObject(x)?x:Current(1),ea:=xml.ea(current),MainFile:=ea.file,TVState(),list:=all?files.sn("//main"):files.find("//main/@file",mainfile,"self::*")
...
Save(option=""){
	...
	for filename in info.2{
		...
		/*
		fl:=FileOpen(filename,"RW",encoding)
		fl.write(text)
		fl.length(fl.position)
		fl.close()
		*/
