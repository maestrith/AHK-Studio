...
Class MainWindowClass{
	...
	__New(win:=1){
		...
		*/
		...
		GUI,Add,Edit,x+5 y0 hwndqf gQFText
		...
		if(ea.type="Search"){
			...
			DllCall("SetWindowPos",int,ctrl,int,0,int,x,int,y,int,w,int,MainWin.FindEditHeight,uint,0x0004|0x0010|0x0020) ;:0x0008|0x0004|0x0010|0x0020
			DllCall("SetWindowPos",int,MainWin.FindTV,int,0,int,x,int,y+MainWin.FindEditHeight,int,w,int,h-MainWin.FindEditHeight,uint,0x0004|0x0010|0x0020) ;:0x0008|0x0004|0x0010|0x0020
		...
		if(ea.type="Tracked Notes"){
			...
			for a,b in [ea.TreeView,ea.Scintilla]
				DllCall("RedrawWindow",int,b,int,0,int,0,uint,0x401|0x2)
			return
		}
		if(last[ctrl].x=x&&last[ctrl].y=y&&last[ctrl].w=w&&last[ctrl].h=h&&ea.type!="Tracked Notes")
			return
		DllCall("SetWindowPos",int,ctrl,int,0,int,x,int,y,int,w,int,h,uint,(ea.type~="Search|Project Explorer|Code Explorer|QF")?0x0004|0x0010|0x0020:0x0008|0x0004|0x0010|0x0020),DllCall("RedrawWindow",int,ctrl,int,0,int,0,uint,0x401|0x2)
		if(ea.type="ToolBar")
		...
		if(type~="i)(Scintilla|Debug)"){
			...
			node:=this.Add(this.NewCtrlPos.hwnd,"Search") ;here
			for a,b in space
				node.SetAttribute(a,b)
		}
...
ExecScript(){
	...
	if(v.Exec.Status=0){
		...
		if(InStr(text,"cannot be opened"))
			return m("Do it again but add #Include %A_ScriptDir% to the script",text)
...
QF(x:=0){
	...
	ControlFocus,,% "ahk_id" MainWin.qf
	...
	ControlGetText,text,,% "ahk_id" MainWin.qf
	if(text)
		goto,qf
	return
	QFText:
