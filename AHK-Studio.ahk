...
Class Icon_Browser{
	...
	__New(id,barid,desc:="",ofile:="Shell32.dll",oicon:=0,return:=0,ahwnd:=""){
		...
		if(this.file~="(.gif|.jpg|.png|.bmp)$")
			...
			count:=0
			while(handle:=LoadPicture(this.file,"icon" A_Index))
				count++
			Loop,%count%
			{
				icon:=IL_Add(il,this.file,A_Index)
				LV_Add("Icon" icon)
			}
			/*
...
New_Segment(new:="",text:="",adjusted:=""){
	...
	/*
		not real sure where the extra file (untitled or otherwise) is coming in when you add a new segment...
	*/
	Sleep,300
	...
	if(fn.code=2002){
		...
		Else{
			...
			if(v.word="#Include"&&v.options.Disable_Include_Dialog!=1)
				...
				Context()
				return
			}else if(node:=ssn(cexml.Find("//main/@file",Current(2).file),"descendant::*[@upper='" upper(v.word) "' and(@type='Class' or @type='Function')]")){
				if(ssn(node,"@type").text="class"){
					SetTimer,AutoClass,-100
				}else if(ssn(node,"@type").text="function")
					SetTimer,AutoParen,-40
			}else
				SetTimer,AutoMenu,-100
	}}if(code=2007)
...
Publish(return=""){
	sc:=csc(),text:=update("get").1,save(),mainfile:=ssn(current(1),"@file").text,publish:=Update({encoded:mainfile}),includes:=sn(current(1),"descendant::*/@include/..")
