...
OR PERFORMANCE OF THIS SOFTWARE. 
	...
	setup(11),Hotkeys(11,{"Esc":"11GuiClose"}), Version:="1.002.30"
	Gui,Margin,0,0
...
Jump_To_First_Available(){
	...
	if(RegExMatch(line,"Oi)^\s*\x23include\s*(.*)(\s*;.*)?$",found))
		...
		if(sc.2007(sc.2267(sc.2008,1))=40){
...
Notify(csc:=""){
	...
	if(fn.code=2001){
		...
		if((StrLen(word)>1&&sc.2102=0&&v.options.Disable_Auto_Complete!=1)){
			if((sc.2202&&v.options.Disable_Auto_Complete_While_Tips_Are_Visible=1)||(sc.2010(cpos)~="\b(13|1|11|3)\b"=1&&v.options.Disable_Auto_Complete_In_Quotes=1)){
				...
				;code_explorer.varlist[current(2).file] looks like a future feature
				if(v.words[sc.2357])
	...
	if(fn.code=2014){
		if(fn.listtype=1){
			...
			static methods
			text:=StrGet(fn.text,"utf-8"),start:=sc.2266(sc.2008,1),end:=sc.2267(sc.2008,1),sc.2645(start,end-start),sc.2003(sc.2008,text "."),sc.2025(sc.2008+StrLen(text ".")),methods:="",node:=cexml.Find("//main/@file",Current(2).file,"descendant::info[@type='Class' and @upper='" Upper(text) "']/*[@type='Method']")
