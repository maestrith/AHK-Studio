...
BraceSetup(Win:=1){
	...
	*/
	...
	while(aa:=all.item[A_Index-1]),ea:=XML.EA(aa){
		...
		Catch,e
			m(e.message,aa.xml)
		v.AutoAdd[SubStr(ea.trigger,0)]:=ea.add,v.BraceMatch[ea.add]:=SubStr(ea.trigger,0),v.MatchBrace[SubStr(ea.trigger,0)]:=ea.add
		v.BraceDelete[SubStr(ea.trigger,0)]:=ea.add,v.DeleteBrace[ea.add]:=SubStr(ea.trigger,0)
