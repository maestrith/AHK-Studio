...
/*
	...
	Run1e{
		dragging an episode of GoT onto the gui crashed it
	}
*/
ComObjError(0),FileCheck(%1%),Options("startup"),menus:=new XML("menus","Lib\Menus.xml"),Keywords(),new Omni_Search_Class(),Gui(),DefaultRCM()
return
WinPos(hwnd){
...
OR PERFORMANCE OF THIS SOFTWARE. 
	...
	Setup(11),Hotkeys(11,{"Esc":"11Close"}), Version:="1.002.39"
	Gui,Margin,0,0
...
Testing(){
	...
	/*
		current:=Current(5)
		all:=SN(current,"descendant::*[@type='Breakpoint']")
		while(a:=all.item[A_Index-1]),ea:=XML.EA(a){
			m(a.xml)
		}
		return
	*/
	...
	*/
	/*
		I need to make a new rcm entry for "Debug"
		Close
		and also
		remove the Default menu set for the other window types
	*/
	/*
		tick:=A_TickCount
		start:=sc.2166(sc.2008)
		if(sc.2225(start)<0){
			while((flan:=sc.2225(--start))<0){
			}
		}
	*/
	/*
...
BraceSetup(Win:=1){
	...
	*/
	...
	for a,b in setup{
		...
		if(a!=b)
			Hotkey,%b%,Brace,On
		if(node:=settings.Find("//autoadd/key/@trigger",a))
			v.AutoAdd[a]:=b
		v.BraceMatch[a]:=b,v.MatchBrace[b]:=a
...
Class MainWindowClass{
	...
	__New(win:=1){
		...
		*/
		...
		top:=this.TopY
		controls:=this.GUI.SN("//win[@win='1']/descendant::control")
		win:=this.WinPos(this.hwnd,1)
		/*
			;here!!!!!!!!!!!!!!!!!!
		*/
		this.Pos()
		add:=v.Options.Top_Find?21:0
...
DefaultRCM(){
	static all:={Scintilla:"Undo,Redo,Copy,Cut,Paste,Select All,Close,Delete,Open,Open Folder,Omni Search"
		    ...
		    ,"Project Explorer":"New,Close,Open,Rename Current Segment,Remove Segment,Copy File Path,Copy Folder Path,Open Folder,Hide/Show Icons,File Icon,Folder Icon,Hide/Show File Extensions,Refresh Project Explorer"
...
EditHotkey(node,window){
	...
	nw:=new GUIKeep("Edit_Hotkey"),nw.add("Hotkey,w240 vhotkey gEditHotkey","Edit,w240 vedit gCustomHotkey","ListView,w240 h220,Duplicate Hotkey Definitions","Button,gEdit_HotkeyEscape Default,Set Hotkey,y"),nw.show("Edit Hotkey")
	return
	EditHotkey:
...
Enter(){
	...
	ControlGetFocus,Focus,% hwnd([1])
	checkqf:
	sc:=csc(),fixlines:=[],ind:=settings.Get("//tab",5)
	if(InStr(focus,"scintilla")){
		...
		if(aa.NodeName="folder"){
			...
			if(SSN(aa.ParentNode,"@file").text="Libraries"){
				if(!cexml.SSN("//main[@file='Libraries']/@tv"))
					Libraries:=TVC.Add(1,"Libraries",0)
...
FileCheck(file:=""){
	...
	,scidate:=20160512235500,XMLFiles:={menus:[20160719203659,"lib/menus.xml","lib\Menus.xml"],commands:[20160508000000,"lib/commands.xml","lib\Commands.xml"]}
	...
	,DefaultOptions:="Smart_Delete,Manual_Continuation_Line,Full_Auto,Focus_Studio_On_Debug_Breakpoint,Word_Wrap_Indicators,Context_Sensitive_Help,Auto_Complete,Auto_Complete_In_Quotes,Auto_Complete_While_Tips_Are_Visible"
	/*
...
Find_Replace(){
	...
	/*
		needs fixed:{
			single replacement (not all) does not work
		}
	*/
	...
	list:=info.segment?SN(Current(),"self::*"):SN(Current(1),"descendant::file")
...
Gui(){
	...
	if(!settings.SSN("//autoadd")){
		...
		for a,b in StrSplit("Smart_Delete,Manual_Continuation_Line,Full_Auto,Focus_Studio_On_Debug_Breakpoint,Word_Wrap_Indicators,Context_Sensitive_Help,Auto_Complete,Auto_Complete_In_Quotes,Auto_Complete_While_Tips_Are_Visible",",")
			opt.SetAttribute(a,b)
	...
	Enter:={Delete:"Delete",Backspace:"Backspace","~Escape":"Escape","^a":"SelectAll","^v":"Paste",WheelLeft:"ScrollWheel",WheelRight:"ScrollWheel","~Ctrl":"ToggleDuplicate"},Hotkeys(1,Enter)
...
class GUIKeep{
	...
	Show(name,position:="",NA:=0,Select:=0){
		static defpos,pos,sel
		defpos:=position,this.GetPos(),pos:=this.resize=1?"":"AutoSize",this.name:=name,sel:=Select
		if(this.resize=1)
			Gui,% this.win ":+Resize"
...
NewIndent(indentwidth:=""){
	...
	*/
	if(indentwidth){
		...
		return sc.Enable(1)
	}if(braces&&!SegmentOpen)
		WinSetTitle(1,files.ea("//*[@sc='" sc.2357 "']"),1),SegmentOpen:=1
	else if(!braces&&SegmentOpen)
		WinSetTitle(1,files.ea("//*[@sc='" sc.2357 "']")),SegmentOpen:=0
...
New_Segment(){
	...
	AddSegment(Filename,text,{start:pos,end:pos})
}
New(filename:="",text:=""){
	...
	*/
	...
	while(ll:=list.Item[A_Index-1],b:=xml.ea(ll)){
		...
		if(fsearch){
			...
			b.node:=ll,LV_Add("",b[info.1],b[info.2],(b[info.2]="Method"?b.class " : ":"") (info.3="file"?Trim(StrSplit(b[info.3],"\").pop(),".ahk"):b[info.3]),b[info.4],rating,++Index),Select[Index]:=b
	}
	...
	if(InStr(search,"?")){
		...
		if(item.type~="Bookmark|Breakpoint"){
			sc:=csc(),text:=sc.GetUNI()
			pre:=SN(item.node,"preceding-sibling::*[@type='" item.type "' and @text='" item.text "']")
			syntax:=v.OmniFindText[item.type],pos1:=1
			Loop,% 1+pre.length
...
WinSetTitle(win:=1,title:="AHK Studio",Open:=0){
	if(IsObject(title)){
		WinSetTitle,% hwnd([win]),,% (open?"Segment Open!  -  ":"") "AHK Studio - " (Current(3).edited?"*":"") (title.dir "\" (v.options.Hide_File_Extensions?title.nne:title.filename))
	}else
...
/*
	...
	ok, rules.
	If at any time indent goes to 0
		stop
	if it gets to the end and there is still an indent
		change the WinSetTitle(1,"OPEN SEGMENT!!!!")
...
Rename_Current_Segment(current:=""){
	...
	FileSelectFile,Rename,,% ea.file,Rename Current Segment,*.ahk
	if(ErrorLevel)
		return
...
Remove_Segment(){
	...
	if(m("Are you sure you want to remove this Segment?","btn:yn","def:2")="no")
		return
...
Check_For_Update(startup:=""){
	...
	time+=sub,hh
	ea:=settings.ea("//github"),token:=ea.token?"?access_token=" ea.token:"",url:="https://api.github.com/repos/maestrith/AHK-Studio/commits/flan" token,http:=ComObjCreate("WinHttp.WinHttpRequest.5.1"),http.Open("GET",url)
	if(proxy:=settings.ssn("//proxy").text)
		http.setProxy(2,proxy)
...
class debug{
	...
	__New(){
		...
		DllCall("ws2_32\freeaddrinfo","ptr",next),DllCall("ws2_32\WSAAsyncSelect","ptr",socket,"ptr",A_ScriptHwnd,"uint",0x9987,"uint",0x29),ss:=DllCall("ws2_32\listen","ptr",socket,"int",32),debug.socket:=socket,debug.filename:=Current(2).file,debug.id:=Current(2).id,debug.Hotkeys(1)
	}Hotkeys(State){
		state:=state?"On":"Off"
	...
	Run(filename){
		...
		while(!WinExist("ahk_pid" pid)){
			Sleep,100
			if(A_Index=5){
				return m("Debugger failed, Please close all instances of " SplitPath(v.debugfilename).filename " and try again")
		...
		if(WinExist("ahk_pid" pid)){
			...
			sc:=csc(),info:=StripError(text,v.debugfilename) ;IMPORTANT
...
Run_Program(){
	if(!debug.socket)
		return Run() ;,debug.Off()
	/*
		sc:=v.debug
		lastline:=sc.GetLine((line:=sc.2166(sc.2006)-1))
		if(InStr(lastline,"press")&&InStr(lastline,"to continue"))
			sc.2645(sc.2167(line),sc.2167(line+1)-sc.2167(line))
	*/
	debug.Send("run")
}
GetChildren(){
...
Display(PopulateVarBrowser:=0){
	...
	while(store:=v.displaymsg.pop()){
		...
		if(info.NodeName="stream"){
			...
			while,bb:=bp.item[A_Index-1],bpea:=xml.ea(bb)
				ad.Insert("breakpoint_set -t line -f " bpea.filename " -n" bpea.line+1)
			for a,b in ad
				v.afterbug.Insert(b)
	...
	while(info:=v.afterbug.pop())
		debug.Send(info)
	InsertDebugMessage(),v.ready:=1,debug.Focus(),debug.Caret(1)
	return
	GetContextInfo:
	debug.Send("stack_get")
	return
