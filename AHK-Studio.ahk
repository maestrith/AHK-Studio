...
class SettingsScintilla{
	__New(window,info){
		...
		notify:=info.notify
		win:=window?window:1
		pos:=info.pos?info.pos:"x0 y0 w0 h0"
		if(info.hide)
			pos.=" Hide"
		mask:=0x10000000|0x400000|0x40000000
		Gui,%win%:Add,custom,%pos% classScintilla +%mask% hwndsc gThemeNotify ;g%notify% ; +1387331584
		for a,b in {fn:2184,ptr:2185}
			this[a]:=DllCall("SendMessageA",UInt,sc,int,b,int,0,int,0)
		this.parent:=sc,this.sc:=sc+0,s.ctrl[sc]:=this
		for a,b in [[2563,1],[2565,1],[2614,1],[2124,1]]
	...
	*/
	__New(option:=0){
		...
		*/
		...
		;hotkeys:=[]
		;hotkeys.Delete:="SWDelete",hotkeys.Backspace:="SWDelete",hotkeys.Enter:="SWEnter"
		...
		;m(SettingsWindow.theme.sc)
		sc:=SettingsWindow.theme
		hwnd:=sc.sc
		sc.ptr:=0,sc.fn:=0,sc:=[]
		DllCall("DestroyWindow",ptr,hwnd)
		GuiControl,Settings:+g,% SettingsWindow.theme.sc
		...
		out=%header%/*`r`n`tMulti-Line`r`n`tcomments`r`n*/`r`n`r`nMain Selection - Multiple Selection`n`nMatching Brace Highlight Sample () <----Edited in the TreeView to the Left with Brace Match *`r`n`r`nSelect the text to change the colors`nThis is a sample of normal text`n`"incomplete quote`n"complete quote"`n`;comment`n0123456789`n[]^&*()+~#\/!`,{`}``b``a``c``k``t``i``c``k`n
		out.="( ,,,, )`n[ ,,,, ]`n{ ,,,, }`n"
		...
		tt:=SettingsWindow.theme.GetText()
		paren:=InStr(tt,"()")
		main:=InStr(tt,"main selection")-1
		mult:=InStr(tt,"multiple selection")-1
		...
		theme.2409(151,1)
		theme.2409(152,1)
		theme.2032(paren-1,152),theme.2033(4,152),
		normal:=InStr(tt,"This is a sample of normal text")-1
		theme.2032(normal,5),theme.2033(31,5)
		theme.2351((start:=StrLen(SubStr(tt,1,InStr(tt,"(")))),start-1)
