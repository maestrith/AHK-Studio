...
OR PERFORMANCE OF THIS SOFTWARE. 
	...
	Setup(11),Hotkeys(11,{"Esc":"11Close"}), Version:="1.003.15"
	Gui,Margin,0,0
...
Class SettingsClass{
	...
	__New(Tab:=""){
		...
		*/
		...
		static ILOBJ:=[],init:=0,IL
		if(!init)
			this.Default("MenuTV"),IL:=IL_Create(50,10),tv_SetImageList(IL),init:=1,IL_Add(IL,file,50)
		...
		if(SettingsClass.Tabs.Menus=tab){
			xx:=SettingsClass.HotkeyXML,action:=xx.SSN("//*[@k='" A_ThisHotkey "']/@a").text,SettingsClass.Default("MenuTV"),ea:=xml.EA(node:=menus.SSN("//*[@tv='" (tv:=TV_GetSelection()) "']"))
			if(action~="MSIU|MSID"){
		...
		ea:=xml.EA(node)
		return opt:=(ea.check?"Check":"") (ea.filename?" Icon" SettingsClass.ILAdd(ea.filename,ea.icon):"")
	}UpdateSavedThemes(){
...
Notifications(a*){
	...
	if(A_GuiControl="MenuTV"){
		if(A_GuiEvent="K"&&A_EventInfo="32")
			this.Default("MenuTV"),node:=menus.SSN("//*[@tv='" TV_GetSelection() "']")
	...
	if(A_GuiControl="Icon"){
		...
		if(node.NodeName="separator")
			return m("Separators can not have icons")
		TV_Modify(TV_GetSelection(),"Icon" this.ILAdd(a.1.file,a.1.icon))
