...
OR PERFORMANCE OF THIS SOFTWARE. 
	...
	setup(11),Hotkeys(11,{"Esc":"11GuiClose"}), Version:="1.002.36"
	Gui,Margin,0,0
...
NewIndent(indentwidth:=""){
	...
	for a,text in code{
		...
		first:=SubStr(text,1,1),last:=SubStr(text,0,1),ss:=(text~="i)^\s*(&&|\bOR\b|\bAND\b|\.|\,|\|\||:|\?)\s*"),indentcheck:=(RegExMatch(text,"iA)}*\s*#?\b(" v.indentregex ")\b",string)&&last!="}")
		if(first="<"){
			Continue
		}
		if(InStr(string,"try"))
...
Wrap_Word_In_Quotes(){
	sc:=csc(),sc.2078,cpos:=sc.2008,start:=sc.2266(sc.2008,0),end:=sc.2267(sc.2008,0),sc.2003(start,Chr(34)),sc.2003(end+1,Chr(34)),sc.2025(cpos+1),sc.2079
}