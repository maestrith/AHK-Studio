...
OR PERFORMANCE OF THIS SOFTWARE. 
	...
	setup(11),Hotkeys(11,{"Esc":"11GuiClose"}), Version:="1.002.35"
	Gui,Margin,0,0
...
Find(){
	...
	if(InStr(button,"search")){
		...
		while,l:=list.item(A_Index-1){
			;out:=Update({encoded:l.text})
			out:=Update({get:l.text})
			found:=1,r:=0,fn:=l.text
			SplitPath,fn,file
...
Refresh_Current_Project(file:=""){
	Extract(Current(2).file),ScanFiles(),FEUpdate(main,1)
}
Refresh_Plugins(){
	Plug(1)
}
