...
if(!gui.ssn("//controls")){
	first=<gui><controls win="1"><control hwnd="2" parent="2" type="Scintilla" lp="0.0000000000" tp="0.0000000000" file="Untitled.ahk" ba="4" ra="3"></control><control hwnd="3" parent="3" type="Project Explorer" lp="0.75" tp="0" la="2" ba="4"></control><control hwnd="4" parent="4" type="Tracked Notes" lp="0" tp="0.75" ta="2"></control></controls></gui>
	gui.xml.LoadXML(first)
}
...
OR PERFORMANCE OF THIS SOFTWARE. 
	...
	setup(11),Hotkeys(11,{"Esc":"11GuiClose"}), Version:="1.002.31"
	Gui,Margin,0,0
...
Context(return=""){
	...
	Loop{
		...
		if((syntax:=commands.ssn("//Commands/commands[text()='" word "' or text()='" word "']/@syntax").text)&&word!="if")
			synmatch.push(word " " syntax),args:=syntax
		else if((syntax:=commands.ssn("//Commands/commands[text()='#" word "' or text()='" word "']/@syntax").text)&&word!="if")
			synmatch.push("#" word " " syntax),args:=syntax
	...
	RegExReplace(args,",","",count)
	syntax:=RegExReplace(syntax,Chr(96) "n","`n")
	if(word="if")
		sc.2207(0xAAAAAA),sc.2200(startpos,syntax)
	else{
		;ff:=RegExReplace(synbak,"\(",",")
		if(commas=0){
			t(syntax,args,count,commas,"START")
			;this is anything RegExMatch(<here>)
		}else if(commas<count){
			t(syntax,args,count,commas,"INSIDE")
			;somewhere in RegExMatch(first,second,<here>,fourth)
		}else if(commas>count&&InStr(args,"*")){
			t(syntax,args,count,commas,"IN THE *")
			;this will be anything that has a * at the end so highlight the last part
		}else if(commas>count&&!InStr(args,"*")){
			t(syntax,args,count,commas,"ERROR!!!!")
			;this is an error state
		}else if(commas=count){
			t(syntax,args,count,commas,"LAST")
			;this will be RegExMatch(first,second,third,<here>)
		}
		/*
			ONLY highlight the text that is in args, don't go outside that.
		*/
		sc.2207(0xff0000),sc.2200(startpos,syntax)
		/*
			WORK ON THE COMMAS AFTER RegExMatch(,,<these commas are ok>","<HERE AFTER IS messed UP>,,) anything after "" in a InStr(thing)
		*/
		/*
			if(commas+1<=count)
...
Notify(csc:=""){
	...
	if(fn.code=2014){
		if(fn.listtype=1){
			...
			text:=StrGet(fn.text,"utf-8")((move:=sc.2007(sc.2008)=40)?"":"()")
			start:=sc.2266(sc.2008,1)
			end:=sc.2267(sc.2008,1)
			sc.2645(start,end-start)
			sc.2003(sc.2008,text)
			sc.2025(sc.2008+StrPut(text,"UTF-8")-(move?0:2))
