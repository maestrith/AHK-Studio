...
OR PERFORMANCE OF THIS SOFTWARE. 
	...
	Setup(11),Hotkeys(11,{"Esc":"11Close"}), Version:="1.003.1"
	Gui,Margin,0,0
...
FileCheck(file:=""){
	...
	,scidate:=20160512235500,XMLFiles:={menus:[20160721074849,"lib/menus.xml","lib\Menus.xml"],commands:[20160508000000,"lib/commands.xml","lib\Commands.xml"]}
...
Class SettingsWindow{
	/*
		make it so that if something calls to it and it doesn't exist, it will make it open
		if not it will:
			create the window
		TAB WILL BE W0 H0 AND AT POINT 0,0 controlled by a listview/treeview....not sure which
		separate tab for the icons to keep the main page clean
		Make a button with accelerator to show the icons list
		have it so when you push that button it will automatically populate a text that says something like{
			you are editing the icon for "X"
		}
		add the toolbar editor to this as well.
	*/
	/*
		Menus selection flow{
			ok, I think I have it now :)
			so now re-arange menu items and such :)
		}
	*/
	/*
		DON'T FORGET THE ICONS TAB
	*/
	__New(option:=0){
		;obj:=new GUIKeep()
		Gui,settings:Destroy
		sww:=SettingsWindow.Windows:=[],obj:=SettingsWindow.win:=new GuiKeep("Settings"),obj.Add("TreeView,x0 y0 w300 h600 AltSubmit gSettingsWindow.Tab,,h","Tab,x+M y0 w0 h0 Section,1|2|3|4|5|6")
		...
		if(Focus="SysTreeView322"){
			...
			if(SSN(node,"descendant::menu"))
				...
				if(m("Are you sure you want to remove this menu?","btn:ync","def:2")="Yes")
					node.ParentNode.RemoveChild(node),SettingsWindow.PopulateMenus()
		}if(Focus="SysListView322"){
			;Menu Items (Hide)
			m("Hide/Show this menu item unless it is a <Separator> then delete it")
		}
		return
	}Disable(){
		for a,b in SettingsWindow.ControlList{
		...
		if(SSN(node,"descendant::*|@personal")){
			...
			while(ll:=list.item[A_Index-1]),ea:=XML.EA(ll)
				if(!SSN(ll,"descendant::*")&&!ea.personal)
					index:=ItemList[ea.clean]:=LV_Add((ea.select?"Select":"") " " SettingsWindow.Icon(ea),RegExReplace(ea.clean,"_"," "),Convert_Hotkey(ea.hotkey),(ea.hide?"Hidden":"Visible")),il[index]:=ll
		}else{
		...
		Default("SysTreeView322","Settings"),TV_Delete(),list:=menus.SN("//menu/descendant-or-self::*") 
		while(ll:=list.item[A_Index-1]),ea:=XML.EA(ll){
		...
		if(focus="SysTreeView322"){
			...
			m(focus,"Change Icon for this control","Set the text on the icon browser tab to reflect the menu item")
...
/*
	Settings(info:=""){
		...
		Default("SysListView322","Settings")
		LV_SetImageList(il:=IL_Create(1,1))
		IL_Add(il,"Shell32.dll",50)
		hotkeys:=[]
		icons:=[]
		TV_SetImageList(il)
		Gui,Settings:ListView,SysListView323
		CommandList:={"Add A New Menu":["!A","ANM"],"Move Menu Item Up":["^Up","Up"],"Move Menu Item Down":["^Down","Down"],"Add Separator":["!S","AS"],"Edit Hotkey":["Enter","EH"],"Re-Load Defaults":["","RD"],"Sort Menus":["","SM"],"Sort Selected Menu":["","SSM"],"Remove Icon":["!^x","RI"],"Remove All Icons From Current Menu":["","RAICM"],"Move Selected":["^M","MS"],"Show Menu Tab":["!M","SMT"],"Show Options Tab":["!O","SOT"],"Add New Sub-Menu":["!U","ANSM"],"Remove All Icons":["","RAI"],"Delete/Hide Items":["Delete","SettingsDelete"]}
		for a,b in CommandList
			LV_Add("",a,Convert_Hotkey(b.1)),hotkeys[b.1]:=b.2
		Loop,3
			LV_ModifyCol(A_Index,"AutoHDR")
		...
		rem:=menus.SN("//*[@tv or @index]"),Default("SysTreeView322","Settings"),menus.SSN("//*[@tv='" TV_GetSelection() "']").SetAttribute("last",1)
		while(rr:=rem.item[A_Index-1])
			rr.R
		emoveAttribute("tv"),rr.RemoveAttribute("index")
		return
		PopulateMenu:
	...
	Loop,% sc.2570{
		...
		if(!v.BraceDelete[chr]&&!v.DeleteBrace[chr]){
			...
			if(match=Chr(sc.2007(cpos+1)))
				...
				bracematch:=sc.2353(cpos),sc.2645(bracematch,1),sc.2645(cpos,1),sc.2584(A_Index-1,cpos),sc.2586(A_Index-1,bracematch-1)
