...
if(!gui.ssn("//controls"))
	gui.xml.LoadXML("<gui><controls win=""1""><control hwnd=""2"" parent=""2"" type=""Scintilla"" lp=""0.0000000000"" tp=""0.0000000000"" file=""Untitled.ahk"" ba=""4"" ra=""3""></control><control hwnd=""3"" parent=""3"" type=""Project Explorer"" lp=""0.75"" tp=""0"" la=""2"" ba=""4""></control><control hwnd=""4"" parent=""4"" type=""Tracked Notes"" lp=""0"" tp=""0.75"" ta=""2""></control></controls></gui>")
...
OR PERFORMANCE OF THIS SOFTWARE. 
	...
	setup(11),Hotkeys(11,{"Esc":"11GuiClose"}), Version:="1.002.34"
	Gui,Margin,0,0
...
Class Code_Explorer{
	...
	Scan(node){
		...
		ea:=xml.ea(node),text:="`n" update({get:ea.file}),pos:=1,next:=cexml.ssn("//file[@file='" (parent:=ssn(node,"@file").text) "']"),fnme:=ea.file
		while,uu:=ssn(next,"*")
		...
		if(node=1){
			all:=cexml.sn("//main")
			while(aa:=all.item[A_Index-1]),ea:=xml.ea(aa){
				obj:=list[ea.file]:=[],CF:=sn(aa,"descendant::*[@type='Class' or @type='Function' or @type='Instance']")
...
Class Icon_Browser{
	...
	things(){
		...
		;while,icon:=IL_Add(il,this.file,A_Index-1){
			;LV_Add("Icon" icon)
		;}
...
class s{
	...
	__New(window,info){
		...
		if(code="enable"){
			...
			return StrGet(&text,vv,files.ssn("//*[@sc='" this.2357 "']/@encoding").text)
		}
...
class SGUI{
	...
	Size(name:=0){
		...
		all:=SGUI.xml.sn("//gui[@gui='" name "']/control")
		while(aa:=all.item[A_Index-1]),ea:=xml.ea(aa){
			if(ea.move)
				...
				for a,b in {x:ea.x,y:ea.y,w:ea.w,h:ea.h}
					if(b!=""){
						GuiControl,%name%:MoveDraw,% ea.hwnd,% a (a~="y|h"?pos.h:pos.w)+b
	}}}
	Show(name){
	...
	__New(win,parent,id,name,mask=""){
		...
		if(code=-20){ ;left click
			...
			if(GetKeyState("Ctrl","P")&&button){
				Toolbar_Editor({hwnd:this.hwnd,id:this.id,button:NumGet(A_EventInfo+12)})
...
Context(return=""){
	...
	Loop{
		...
		if(word){
			...
			if((ea:=scintilla.ea("//scintilla/commands/item[@code='" word "']")).syntax){
				...
				}else if(node:=cexml.ssn("//lib/descendant::info[@upper='" Upper(word) "']"))
					start:="new " word,args:=ssn(node,"descendant::*[@type='Method' and @upper='__NEW']/@args").text
				...
				start:=word
			else if(fun:=cexml.ssn("//lib/descendant::info[@upper='" Upper(word) "']"))
				...
				return
			new:=StrSplit(args,Chr(96) "n"),NewString:=((main:=start "(" new.1 ")") RegExReplace(RegExReplace(args,"\Q" new.1 "\E"),Chr(96) "n","`n")),sc.2207(0xff0000),sc.2200(wordstartpos,newstring),RegExReplace(args,",",,Count),len:=StrPut(start,"UTF-8")
			if(commas=0)
				sc.2204(len,StrPut(SubStr(main,1,InStr(main,",")-1),"UTF-8")-1)
			else if(commas>0&&commas<count)
				sc.2204(StrPut(SubStr(newstring,1,InStr(newstring,",",0,1,commas)))-1,StrPut(SubStr(newstring,1,InStr(newstring,",",0,1,commas+1)))-2)
			else if(commas>0&&commas>=count){
				info:=StrSplit(args,",")
	...
	if(word="if")
		...
		if(commas=0){
			...
			sc.2204(len+InStr(adj.1,",",0,1,commas),len+InStr(adj.1,",",0,1,commas+1)-1)
		}else if(commas>count)
...
defaultfont(){
	temp:=new xml("temp")
	info=<fonts><author>joedf</author><font background="0x222827" bold="1" color="0xF8F8F2" font="Consolas" size="10" style="5" italic="0" strikeout="0" underline="0"></font><font background="0x2D3534" style="33" color="0xECEEEE"></font><font code="2069" color="0xFF8080"></font><name>Monokai</name><font style="13" color="0x2929EF" background="0x222827"></font><font style="3" color="0x74DBE6"></font><font style="1" color="0x5E7175" bold="0" font="Consolas" italic="1" size="10" strikeout="0" underline="0"></font><font style="2" color="0xFF81AE"></font><font style="4" color="0xAFA600"></font><font style="15" color="0x2D8BAA"></font><font style="18" color="0xEFD966"></font><font style="19" color="0x0000AE"></font><highlight><list1 list="1">custom list</list1></highlight><font style="22" color="0x8000FF"></font><font style="21" color="0x400080"></font><font style="11" color="0x5E7175" bold="0" font="Consolas" italic="1" size="10" strikeout="0" underline="0"></font><font style="17" color="0x9A93EB"></font><font bool="1" code="2068" color="0x3E4849"></font><font bool="1" code="2067" color="0xECE000"></font><font code="2098" color="0x4E5460"></font></fonts>
...
EditHotkey(node,window,Control:=""){
...
Encode(tt,encoding:="UTF-8"){
	len:=StrPut(tt,encoding)
	length:=VarSetCapacity(text,len)
...
Extract(mainfile){
	...
	if(encoding="utf-16"){
		rem:=files.Find("//@file",file),rem.ParentNode.RemoveChild(rem)
		goto,ExtractBottom
	}
...
FileCheck(file){
	static dates:={commands:{date:20151222093855,loc:"lib\commands.xml",url:"lib/commands.xml",type:1},menus:{date:20160403125250,loc:"lib\menus.xml",url:"lib/menus.xml",type:2},scilexer:{date:20160328074923,loc:"SciLexer.dll",url:"SciLexer.dll",type:1},icon:{date:20150914131604,loc:"AHKStudio.ico",url:"AHKStudio.ico",type:1},Studio:{date:20151021125614,loc:A_MyDocuments "\Autohotkey\Lib\Studio.ahk",url:"lib/Studio.ahk",type:1}},url:="https://raw.githubusercontent.com/maestrith/AHK-Studio/master/"
...
Find(){
	...
	if(InStr(button,"search")){
		...
		while,l:=list.item(A_Index-1){
			out:=Update({encoded:l.text}),found:=1,r:=0,fn:=l.text
			SplitPath,fn,file
...
Class GuiKeep{
	...
	__New(win,info*){
		...
		this.xml.Under(this.xml.ssn("//*"),"control",{type:opt.1,hwnd:hwnd})
		return hwnd
	}save(win){
		if(win)
			SaveGUI(win)
...
Jump_To_First_Available(){
	...
	if(RegExMatch(line,"Oi)^\s*\x23include\s*(.*)(\s*;.*)?$",found))
		...
		word:=Upper(sc.GetWord())
		if(SubStr(word,1,1)="g"&&node:=cexml.ssn("//*[@upper='" Upper(SubStr(word,2)) "' and(@type='Label' or @type='Function')]"))
			return ea:=xml.ea(node),tv(xml.ea(files.Find("//file/@file",ssn(node,"ancestor-or-self::file/@file").text)).tv,{start:ea.pos,end:ea.pos+StrPut(word,"UTF-8")-2})
		if(sc.2007((pos:=sc.2266(sc.2008,1)-1))=46){
			word2:=Upper(sc.textrange(sc.2266(pos-1,1),sc.2267(pos-1,1)))
			if(node:=cexml.ssn("//main[@file='" Current(2).file "']/descendant::*[@upper='" word2 "' and @type='Instance']")){
				if(node:=cexml.ssn("//main[@file='" Current(2).file "']/descendant::*[@upper='" word "' and @class='" ssn(node,"@class").text "' and @type='Method']"))
					total.=(info:=ssn(node,"@type").text " " StrSplit(ssn(node,"ancestor-or-self::file[@file]/@file").text,"\").pop()) "|",v.jtfa[info]:=node
			}else if((list:=cexml.sn("//main[@file='" Current(2).file "']/descendant::*[@upper='" word "' and @type='Method']")).length){
				while(ll:=list.item[A_Index-1]),ea:=xml.ea(ll)
					total.=(info:=ea.type " " StrSplit(ssn(ll,"ancestor-or-self::file[@file]/@file").text,"\").pop()) "|",v.jtfa[info]:=ll
		}}else
			if(node:=cexml.ssn("//main[@file='" Current(2).file "']/descendant::*[@upper='" word "' and @type='Function']"))
				total.=(info:=ssn(node,"@type").text " " StrSplit(ssn(node,"ancestor-or-self::file[@file]/@file").text,"\").pop()) "|",v.jtfa[info]:=node
		if((list:=cexml.sn("//main[@file='" current(2).file "']/descendant::*[@upper='" word "' and @type!='Method' and @type!='Function']")).length)
			while(ll:=list.item[A_Index-1]),ea:=xml.ea(ll)
				total.=(info:=ea.type " " StrSplit(ssn(ll,"ancestor-or-self::file[@file]/@file").text,"\").pop()) "|",v.jtfa[info]:=ll
		if(total:=Trim(total,"|")){
...
Jump_To(type){
	...
	if(RegExMatch(line,"Oi)^\s*\x23include\s*(.*)(\s*;.*)?$",found)){
		/*
			add in <file> and all the %A_ScriptDir% things...
		*/
...
Omni_Search(start=""){
	...
	while,ll:=list.Item[A_Index-1],b:=xml.ea(ll){
		...
		if(b[info.1]){
			item:=LV_Add("",b[info.1],two,b[info.3],v.options.HasKey(b.clean)?(v.options[b.clean]?"Enabled":"Disabled"):b[info.4],rating,LV_GetCount()+1),Select[item]:=b
		}
	}
	Loop,4
...
Save(option=""){
	...
	if(option=1){
		...
		UnSaved()
		/*
			all:=files.sn("//file[@untitled]"),template:=Template()
			while(aa:=all.item[A_Index-1]),ea:=xml.ea(aa){
				text:=Update({encoded:ea.file})
				if(text!=template)
					m("WARNING! NONE OF YOUR UNTITLED DOCUMENTS DATA HAS BEEN SAVED!","In order to continue, all of your Untitled files need to be saved or closed","You WILL lose data if you do not save your Untitled documents.","If this message comes up a lot consider turning on New File Dialog and saving the files when you start them","ico:!")
			}
		*/
	}
	savedfiles:=[],saveas:=[]
	for filename in info.2{
		...
		encoding:=files.ssn("//file[@file='" filename "']/@encoding").text
...
Toolbar_Editor(ctrl){
	static tb,nw,tbid,cross,Visible,Current,all
	if(ctrl.file){
	...
	ea:=xml.ea(ctrl),current:=ea.hwnd,Visible:=[],items:=[],all:=menus.sn("//main/descendant::*"),nw:=new GUIKeep("Toolbar_Editor"),width:=Settings.Get("//IconBrowser/Win[@win='Toolbar_Editor']/@w",300),nw.Add("Edit,w100,color","Text,x+M h23 w50 gNextChecked,Next","ComboBox,x+M gtesearch w200 vedit,,w","ListView,xm w150 h200 AltSubmit,Toolbars,h","TreeView,x+0 w200 h200 Checked AltSubmit,,wh","ListView,x+M200 yM w" width " h180 icon Section gSelectIcon AltSubmit,Icon,xh")
	ControlGet,hwnd,hwnd,,SysListView322,% hwnd(["Toolbar_Editor"])
	new Icon_Browser(nw,hwnd,"Toolbar_Editor","xy",300,"Toolbar_Editor","Toolbar_Editor"),nw.show("Toolbar Editor"),total:="",cross:=[]
	...
	Gui,%winname%:Color,0xff00ff,0xff00ff
...
tv(tv*){
	...
	if(sc.sc=v.tnsc.sc)
		sc:=csc(1)
	if(sel){
		GetPos(),node:=files.ssn("//*[@tv='" sel "']"),ea:=xml.ea(node)
		...
		if(ea.sc)
			...
			if(!ea.sc){
				sc.2358(0,0)
				Sleep,80
				doc:=sc.2357,sc.2376(0,doc),node.SetAttribute("sc",doc),tt:=update({get:ea.file}),txt:=Encode(tt,enc:=ssn(node,"@encoding").text),sc.2037(65001),sc.2181(0,&txt),Update({encoding:enc,file:ea.file}),sc.2175(),Set(sc)
			}else
...
Settings(info:=""){
	...
	if(info.file){
		if(newwin[].TopLevel){
			...
			;ControlFocus,SysTreeView321,% hwnd(["Settings"])
	...
	GuiControl,Settings:+gPopulateMenu,SysTreeView321
	...
	GuiControl,Settings:+g,SysListView322
	GuiControl,Settings:-Redraw,SysListView322
	Default("SysListView322","Settings"),LV_Delete()
	while(ll:=list.item[A_Index-1]),ea:=xml.ea(ll)
	...
	if(A_GuiEvent="I"&&ErrorLevel~="SF"){
		LV_GetText(text,A_EventInfo),node:=menus.ssn("//*[@clean='" clean(text) "' and not(@top)]"),parent:=ssn(node.ParentNode,"@clean").text,item:=ssn(node,"@clean").text,Enable("SysTreeView321","","Settings"),Default("SysTreeView321","Settings"),TV_Modify(menus.ssn("//*[@clean='" parent "' and @top]/@tv").text,"Select Vis Focus"),Enable("SysTreeView321","SetOption","Settings")
		Gosub,PopulateMenu
	...
	*/
	...
	return
	Populate_TreeView:
	Gui,Settings:TreeView,SysTreeView321
	Enable("SysTreeView321","","Settings"),TV_Delete(),all:=menus.sn("//descendant::*")
	while,aa:=all.item[A_Index-1],ea:=xml.ea(aa)
	...
	Enable("SysTreeView321","PopulateMenu","Settings"),TV_Modify(TV_GetChild(0),"Select Vis Focus"),all:=menus.sn("//*[@top=1 and @last]")
	while(aa:=all.item[A_Index-1]),ea:=xml.ea(aa)
