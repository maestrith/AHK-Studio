...
Class MainWindowClass{
	...
	SetWinPos(hwnd,x,y,w,h,ea="",flags:="",set:=0){
		y:=v.Options.Top_Find?y+=this.qfheight:y,DllCall("SetWindowPos",int,hwnd,int,0,int,x,int,y,int,w,int,h,uint,flags),DllCall("RedrawWindow",int,hwnd,int,0,int,0,uint,0x401|0x2)
		if(set){
	...
	Split(direction:=0,type:="Scintilla"){
		...
		if(direction="Right") ;here
			this.SetWinPos(hwnd,ea.x,ea.y+add,np.x-ea.x,ea.h,ea,,1),space:={x:np.x,y:ea.y+add,w:ea.w-(np.x-ea.x),h:ea.h}
		if(type~="i)(Scintilla|Debug)"){
			...
			node:=this.Add(this.NewCtrlPos.hwnd,"Search"),win:=this.WinPos()
		...
		for a,b in space
			node.SetAttribute(a,b)
		this.NewCtrlPos:="",this.mousedown:=0,win:=this.WinPos()
		if(space.x)
			node.SetAttribute("lp",Round(space.x/win.w,6))
		if(space.y)
			node.SetAttribute("tp",Round(space.y/win.h,6))
			...
			if((nodes:=files.SN("//*[@id='" ea.id "']")).length>1){
				while(nn:=nodes.item[A_Index-1]),fea:=XML.EA(nn){
					if(fea.sc){
						...
						Break
						;m("It already exists with an SC of " fea.sc)
					}
				}
			}
		}
