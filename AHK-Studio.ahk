	...
	scan(node){
		...
		if(node){
			...
			while(aa:=all.item[A_Index-1]),ea:=xml.ea(aa)
				obj.list.=ea.text " "
			obj.list:=Trim(obj.list)
			;t(++count,A_TickCount-tick,"time:1")
			return
		}else{
...
Extract(mainfile){
	...
	set.SetAttribute("encoding",encoding)
	...
	/*
		...
		Don't delete the cexml and start over.
		Don't delete the cexml for that file and start over either
		Check the cexml to make sure that none of the files that are there haven't been removed.
		If they have
			remove them from cexml and TV_Delete(item)
		re-scan the files but don't Code_Explorer.scan() anything that isn't in the cexml already!
		{
			When you do the extract function have it search the files XML to see if the file already exists in the XML.
			If it does not add scan attribute.
				After doing extract create a function called updatecexml.
			This will go through the files XML end scan all of the files that the scan attribute.
			In code_explorer.scan() have it remove the scan attribute
		}
	*/
	pos:=1
...
Open(filelist="",show="",Redraw:=1){
	...
	if(!filelist){
		...
		code_explorer.Refresh_Code_Explorer(),PERefresh(),v.tngui.Populate()
	}else{
		v.filescan:=[]
...
Rename_Current_Segment(current:=""){
	...
	Update({remove:ea.file}),Save(),Extract(MainFile),FEUpdate(MainFile)
	
		node:=cexml.find("//@file",ea.file)
		node.ParentNode.RemoveChild(node)
		Code_Explorer.scan(files.find("//file/@file",renamete))
