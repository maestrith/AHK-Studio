...
Save(option=""){
	...
	for filename in info.2{
		...
		encoding:=files.ssn("//file[@file='" filename "']/@encoding").text,encoding:=encoding?encoding:"UTF-8",file:=FileOpen(filename,"rw","UTF-8"),file.seek(0),file.write(text),file.length(file.position),file.Close()
	...
	if(sel){
		...
		if(ea.sc)
			...
			if(!ea.sc){
				sc.2358(0,0)
				Sleep,20
				doc:=sc.2357,sc.2376(0,doc),node.SetAttribute("sc",doc),tt:=update({get:ea.file}),sc.2037(65001),txt:=Encode(tt,ssn(node,"@encoding").text),sc.2181(0,&txt),sc.2175(),Set(sc)
			}else
